...
if __name__ == '__main__':
    input_path = "./input/Stroke-Pencil/stroke1.png"
    output_path = "./input/Stroke-Pencil"

    stroke = cv2.imread(input_path, cv2.IMREAD_GRAYSCALE)
    
    (h,w) = stroke.shape
    h*=3
    w*=3
    stroke = cv2.resize(stroke,(w,h))
    ...
    (H,S,V) = (100,250,200)
    # cv2 默认BGR
    pure = Gassian_HSV(size=(h,w,3), mean=[H,S,V], var = [0,0,0])  # hsv 颜色空间测试
    pure = cv2.cvtColor(pure, cv2.COLOR_HSV2BGR)
    cv2.imshow('bgr', pure)


    canvas = Gassian_HSV(size=(h,w,3), mean=[H,S,V], var = [0,0,0])  # hsv 颜色空间测试
    ...
    if 1:
        (B,G,R) = (100,250,200)
        # cv2 默认BGR
        pure = Gassian_HSV(size=(h,w,3), mean=[B,G,R], var = [0,0,0])  # hsv 颜色空间测试
        # pure = cv2.cvtColor(pure, cv2.COLOR_HSV2BGR)
        cv2.imshow('bgr', pure)

        bg = Gassian_HSV(size=(h,w,3), mean=[255,255,255], var = [0,0,0])  # hsv 颜色空间测试
        canvas = Gassian_HSV(size=(h,w,3), mean=[B,G,R], var = [0,0,0])  # hsv 颜色空间测试
        canvas = canvas.astype("float32") * brush[:,:,np.newaxis].astype("float32")/255 + bg*(1.0-brush[:,:,np.newaxis].astype("float32")/255)
        canvas = np.uint8(canvas) 

        # canvas[brush<120,2] = 255
