...
def Getline(distribution, length):
    ...
    if length < constant_length:  # if length is too short, lines are Aligned
        patch = Gassian((2*period, length), mean=255, var = 0) 
        begin = 0
        end = 1
        ...
        patch = Gassian((2*period, length+4*period), mean=255, var = 0) 
...
def Attenuation(patch, period, distribution, begin, end):
    ...
    radius = (period-1)/2
    canvas = Gassian((patch.shape[0], patch.shape[1]), mean=255, var=3)
...
def GetParallel(distribution, height, length, period):
    if length<constant_length: # constant length
        canvas = Gassian((height+2*period,length), mean=255, var = 0)  
    else: # variable length
        canvas = Gassian((height+2*period,length+4*period), mean=255, var = 0)  
...
def ChooseDistribution(period, Grayscale):
    ...
    for i in range(distribution.shape[0]):
        distribution[i][0] = Grayscale + difference*abs(i-c)/c * 0.8
...
if __name__ == '__main__':
    np.random.seed(0)
    canvas = Gassian((500,500), mean=255, var = 0)
    ...
    cv2.waitKey(0)
    cv2.imwrite("D:/ACM2022/2.0/input/Stroke-Pencil/Parallel.png", patch)
    print("done")
